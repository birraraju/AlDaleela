...
import { IoIosArrowForward } from "react-icons/io";// Assuming this component contains the body of the feedback
import { useTheme } from '../../../../../Layout/ThemeContext/ThemeContext'; // Import your theme context
import FilterInnerBody from '../GeneralInformation/FilterInnerBody'
export default function GeneralFilterHeader({setShowInfo,
  width = "454.84px",
  ...
  return (
    <div
      ref={feedbackRef}
      className={`fixed sm:top-20 w-[98%] sm:w-[600px] top-14 ${ isLangArab?" left-3 sm:left-16 laptop_s:left-6":"right-2 sm:right-16 laptop_s:right-6"} transition-transform duration-300 ease-in-out ${
        isOpen ? "translate-x-0" : ( isLangArab?"-translate-x-[104%] sm:-translate-x-[116%] laptop_s:-translate-x-[107%]":" translate-x-[103%] sm:translate-x-[110%] laptop_s:translate-x-[104%]")
      }`}
      style={{  height }}
    >
      <div

        className={`relative h-[620px] sm:h-[400px] laptop_s:h-[400px] sm:w-full sm:float-none w-[99%] float-end rounded-2xl shadow-lg overflow-scroll border ${isLangArab?"laptop_s:sm:h-[625px] ":"laptop_s:sm:h-[620px] "} ${
      ...
      >
        ...
        <div className="p-2 sm:py-2 sm:px-4  overflow-y-auto  h-[85%]">
            <FilterInnerBody/>
        </div>
      </div>
